                       ...
                       uint64_t            end_addr)
{
    ...
    // // ======================================================================
    // // 【新增调试逻辑】验证谁在跑 nvidia.ko
    // // ======================================================================
    // // 获取 IP0 配置的范围 (假设你已在 harness 里把 nvidia.ko 范围设给了 IP0)
    // uint64_t target_start = GET_GLOBAL_STATE()->pt_ip_filter_a[0];
    // uint64_t target_end   = GET_GLOBAL_STATE()->pt_ip_filter_b[0];

    // // 检查当前解码出的指令地址是否落在 nvidia.ko 范围内
    // if (GET_GLOBAL_STATE()->pt_ip_filter_configured[0] &&
    //     start_addr >= target_start && start_addr < target_end) 
    // {
        
    //     nyx_printf("[PT-HIT] ip=0x%lx len=%lu soft_cr3=0x%lx\n",
    //            start_addr,
