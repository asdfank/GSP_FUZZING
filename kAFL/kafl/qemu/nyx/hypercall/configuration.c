                                            ...
                                            uint64_t        hypercall_arg)
{
    ...
    if (read_virtual_memory(vaddr, (uint8_t *)&config, sizeof(agent_config_t), cpu)) {
        ...
        GET_GLOBAL_STATE()->disable_fast_reload = (config.agent_non_reload_mode != 0);
        ...
        // ★★ 建议新增：根据 agent_tracing 和 nyx_pt 选择 PT 模式 ★★
        if (!config.agent_tracing && GET_GLOBAL_STATE()->nyx_pt) {
            // 不用编译时插桩，切换到 PT 覆盖模式
            GET_GLOBAL_STATE()->pt_trace_mode = true;
            nyx_printf("[AGENT CONFIG] Switching to Intel PT coverage mode.\n");
        } else {
            // 否则默认不启用 PT 覆盖（或者由后续逻辑覆盖）
            GET_GLOBAL_STATE()->pt_trace_mode = false;
            nyx_printf("[AGENT CONFIG] Intel PT coverage mode disabled.\n");
        }
