...
{
    // nyx_printf("[PT] Dumping %d bytes of trace\n", bytes);
          ...
          GET_GLOBAL_STATE()->redqueen_state->intercept_mode))
    {
            ...
            GET_GLOBAL_STATE()->decoder && !GET_GLOBAL_STATE()->dump_page)
        {
            ...
            // nyx_printf("[DECODER] Result: %d (success=0, overflow=1, page_fault=2)\n", result);
            if (result != decoder_success) {
                 nyx_printf("[DECODER ERROR] Fault addr: 0x%lx\n", libxdc_get_page_fault_addr(GET_GLOBAL_STATE()->decoder));
            }
            switch (result) {
...
{
    //nyx_printf("[PT-TEST] pt_enable called!\n");  // 用 nyx_printf，因为它无条件输出
...
//     r += pt_cmd(cpu, KVM_VMX_PT_CONFIGURE_CR3, hmp_mode);
//     //r += pt_cmd(cpu, KVM_VMX_PT_ENABLE_CR3, hmp_mode);
//     nyx_printf("[PT-HACK] CR3 filtering DISABLED for system() fuzzing.\n");
//     return r;
// }
...
int pt_set_cr3(CPUState *cpu, uint64_t val, bool hmp_mode)
{
    ...
    uint64_t cpu_cr3 = x86_cpu->env.cr[3];

    // 1. 记录日志，方便在 qemu_nyx.log 里对照
    nyx_printf("[PT-HACK] pt_set_cr3 ENTER: val=0x%lx, cpu_cr3=0x%lx, old_filter=0x%lx\n",
               val, cpu_cr3, GET_GLOBAL_STATE()->pt_c3_filter);

    // 2. 维护软件状态 (pt_c3_filter)
    // 这个变量现在只作为“最后一次谁举手抢了监控权”的标记，不影响硬件
    if (val == 0) {
        GET_GLOBAL_STATE()->pt_c3_filter = cpu_cr3;
        nyx_printf("[PT-HACK] SUBMIT_CR3(0) -> Soft-Tracking current CPU CR3=0x%lx\n",
                   GET_GLOBAL_STATE()->pt_c3_filter);
    } else {
        GET_GLOBAL_STATE()->pt_c3_filter = val;
        nyx_printf("[PT-HACK] SUBMIT_CR3(%lx) -> Soft-Tracking directly\n", val);
    }

    // 3. 【关键】只配置硬件寄存器，但不启用 CR3 过滤 (System-Wide Mode)
    // 这样硬件会记录所有进程的 Trace，无论 CR3 是多少
    r += pt_cmd(cpu, KVM_VMX_PT_CONFIGURE_CR3, hmp_mode);
    ...
    nyx_printf("[PT-HACK] CR3 filtering DISABLED (System-Wide Trace). Soft-Target=0x%lx\n",
           GET_GLOBAL_STATE()->pt_c3_filter);
