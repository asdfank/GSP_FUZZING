...
{
    if (GET_GLOBAL_STATE()->disable_fast_reload) 
    {
        // 完全关闭 fast-reload：不要再刷“根快照不可用”
        return;
    }
...
void synchronization_disable_pt(CPUState *cpu)
{
    ...
    // nyx_printf("[PT] sync_disable_pt: pt_trace_size=%lu, bb_coverage=%u\n",
    //            GET_GLOBAL_STATE()->pt_trace_size,
    //            GET_GLOBAL_STATE()->bb_coverage);
...
{
    int pt_ret = pt_enable(cpu, false);  // 注意：用函数参数 cpu，而不是 qemu_get_cpu(0)
    if (pt_ret == 0) {
        //nyx_printf("[PT] Enabled successfully.\n");  // 成功日志，使用 hprintf 输出到 host
        cpu->pt_enabled = true;
    } else {
        nyx_printf("[PT] Enable failed:.\n");  // 失败日志，输出返回值
    }
