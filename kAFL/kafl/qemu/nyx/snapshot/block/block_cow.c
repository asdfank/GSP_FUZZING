                                           ...
                                           uint64_t         iov_offset)
{
    ...
    if (unlikely(k == kh_end(self->lookup_primary))) {
        ...
        /* ★ 新增：如果 primary 缓冲区已经不够再放一个 CHUNK，就不要再缓存，直接返回 */
        if (self->offset_primary + CHUNK_SIZE > self->cow_primary_size) {
            GET_GLOBAL_STATE()->cow_cache_full = true;
#ifdef COW_CACHE_DEBUG
            nyx_debug("COW primary buffer full (offset=0x%lx size=0x%lx) -> "
                      "skip caching new page for %s\n",
                      (unsigned long)self->offset_primary,
                      (unsigned long)self->cow_primary_size,
                      self->filename);
#endif
            /* 不设置 kh_value，不增加 offset_primary，后续写入直接走底层块设备 */
            return;
        }
        ...
        nyx_debug(
            "COW CACHE IS 0x%lx BYTES (KB: %ld / MB: %ld / GB: %ld) IN SIZE!\n",
            self->offset_primary,
            self->offset_primary >> 10,
            self->offset_primary >> 20,
            self->offset_primary >> 30);
#endif

        /* ★ 删除原来的 abort：缓冲区写满时不再 assert 退出 */
        /* assert(self->offset_primary < self->cow_primary_size); */
    }
                                             ...
                                             uint64_t         iov_offset)
{
    ...
    if (!self->enabled_fuzz_tmp) {
        ...
        /* ★ 缓冲区满了就标记 full，但不再 abort，只是后面不再缓存新的页 */
        if (self->offset_secondary >= COW_CACHE_SECONDARY_SIZE) {
            GET_GLOBAL_STATE()->cow_cache_full = true;
#ifdef COW_CACHE_DEBUG
            nyx_debug("COW secondary buffer full (offset=0x%lx size=0x%lx) -> "
                      "skip caching new page for %s\n",
                      (unsigned long)self->offset_secondary,
                      (unsigned long)COW_CACHE_SECONDARY_SIZE,
                      self->filename);
#endif
            return;
        }
        ...
        /* ★ 同理，TMP 缓冲区满了也只标记，不 abort */
        if (self->offset_secondary_tmp >= COW_CACHE_SECONDARY_SIZE) {
            GET_GLOBAL_STATE()->cow_cache_full = true;
#ifdef COW_CACHE_DEBUG
            nyx_debug("COW secondary TMP buffer full (offset=0x%lx size=0x%lx) -> "
                      "skip caching new page for %s\n",
                      (unsigned long)self->offset_secondary_tmp,
                      (unsigned long)COW_CACHE_SECONDARY_SIZE,
                      self->filename);
#endif
            return;
